...
contract DebridgeZapSilo is Test {
    ...
    function test_bridgeIntoSilo() public {
        ...
        (address protectedShareToken,,) = s_usdc_siloConfig.getShareTokens(address(usdc_Vault));
        uint256 userProtectedShares = IERC20(protectedShareToken).balanceOf(alice);
        uint256 userProtectedAssets = usdc_Vault.previewRedeem(userProtectedShares, ISilo.CollateralType.Protected);
        console.log("alice protected assets", userProtectedAssets);

        assertEq(userProtectedAssets, zapAmount);

        _redeem(userProtectedShares);
        console.log("alice USDC balance", IERC20(USDC).balanceOf(alice));
        assertEq(IERC20(USDC).balanceOf(alice), usdc_initial_amount + zapAmount);
    }
    ...
    function _redeem(uint256 shares) internal returns (uint256 depositAssets) {
        ...
        depositAssets = usdc_Vault.redeem(shares, alice, alice, ISilo.CollateralType.Protected);
        vm.stopPrank();
    }
    ...
    function _prepareExternalCall() internal view returns (bytes memory) {
        bytes memory payload = abi.encode(
            DlnExternalCallLib.ExternalCallPayload({
                to: address(usdc_Vault),
                txGas: 0,
                callData: abi.encodeWithSelector(ISilo.deposit.selector, zapAmount, alice, ISilo.CollateralType.Protected)
            })
        );
